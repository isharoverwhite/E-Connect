...
FIRMWARE_PUBLIC_BASE_URL=https://192.168.8.4:3000
# Optional nếu broker public cho firmware khác host với API/public origin:
FIRMWARE_MQTT_BROKER=192.168.8.20
...
```bash
HTTPS_HOSTS=my-lab.local HTTPS_IPS=192.168.1.10 npm run dev
```
...
- Nếu firmware build vẫn báo không suy ra được reachable host hoặc bạn không thể dùng host networking, đặt `FIRMWARE_PUBLIC_BASE_URL` trên backend thành origin thật của server, ví dụ `https://192.168.8.4:3000` hoặc hostname reverse proxy.
