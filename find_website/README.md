...
Public-facing E-Connect LAN scanner modeled after Synology Web Assistant. The page probes preferred local aliases such as `econnect.local` first, then tries the browser client's current private subnet when available, and only then falls back to broader common private subnets. It launches the HTTPS WebUI from the responding probe host plus the sanitized transport hints returned by the backend, with the standard self-hosted target normalized to `https://<host>:3443`. The secure public host keeps the JSONP-style `http://<candidate-ip>:8000/web-assistant.js?callback=...` transport, while LAN-hosted HTTP copies can probe `/health` directly to avoid browser script-loading deadlocks on the same runtime payload.
...
- For self-hosted Docker Compose deployments, the user stack stays limited to `db`, `mqtt`, `server`, and `webapp`; the recommended way to publish `econnect.local` is the optional `discovery-mdns` profile in `deploy/user/compose.yml` or `docker-compose.user.yml`, which runs `discovery_mdns` from the backend runtime on host networking and should reuse the same LAN IP configured in `https_ips`.
...
- The scanner probes the same HTTPS WebUI it will launch. On a fresh LAN device, an untrusted certificate on `https://<host>:3443` can make the WebUI probe show `offline` even when the stack is healthy.
