...
When Jenkins deploys the Docker stack, it now also auto-resolves a LAN IP for the build node, publishes `econnect.local` through the `discovery_mdns` helper running on host networking, and aligns `FIRMWARE_PUBLIC_BASE_URL`, `FIRMWARE_MQTT_BROKER`, and `HTTPS_HOSTS` with that alias unless the job already overrides them explicitly.
