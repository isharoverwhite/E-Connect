...
ONLINE_CHECK_TIMEOUT_SECONDS = 0.5
...
def _is_yeelight_reachable(host: str) -> bool:
    try:
        conn = socket.create_connection((host, DEFAULT_PORT), timeout=ONLINE_CHECK_TIMEOUT_SECONDS)
        conn.close()
        return True
    except OSError:
        return False
...
def execute_command(device: dict[str, Any], command: dict[str, Any]) -> dict[str, Any]:
    ...
    last_error: ExtensionRuntimeError | None = None
    ...
    for attempt in range(MAX_COMMAND_ATTEMPTS):
        try:
            ...
            if attempt < MAX_COMMAND_ATTEMPTS - 1:
                if exc.connection_failed and not _is_yeelight_reachable(normalized_host):
                    raise exc  # Device is offline — abort retries immediately.
                time.sleep(RETRY_INTERVAL_SECONDS)

    assert last_error is not None

    # All attempts failed. Probe state: command may have reached the lamp despite TCP errors.
    ...
    # Online check: if discovery succeeds the device is reachable and the command truly failed.
    if not last_error.connection_failed:
        discovery = _discover_yeelight_metadata(normalized_host)
        if discovery is not None:
            raise ExtensionRuntimeError(
                f"Yeelight command failed after {MAX_COMMAND_ATTEMPTS} attempts (device is online).",
                mark_offline=False,
            ) from last_error
    raise last_error
...
def _send_yeelight_power_command(
    ...
    *,
    ...
    try:
        session.send("set_power", [desired_power, "smooth", TRANSITION_MS, 0], expect_reply=False)
    except ExtensionRuntimeError:
...
def _apply_yeelight_command(
    ...
    *,
    ...
    if command.action == "color_temperature":
        ...
        session.send(
            "set_ct_abx",
            [command.color_temperature or 4000, "smooth", TRANSITION_MS],
            expect_reply=False,
        )
        return
    ...
    if command.action == "brightness":
        ...
        if requested_brightness <= 0:
            if known_power_state == "off":
                return
            session.send("set_power", ["off", "smooth", TRANSITION_MS, 0], expect_reply=False)
        else:
            if known_power_state == "off":
                _send_yeelight_power_command(session, "on", allow_timeout=True)
            session.send(
...
def _prepare_yeelight_command(
    ...
    *,
    ...
    # misinterpreted as brightness commands.
    if "value" in command:
