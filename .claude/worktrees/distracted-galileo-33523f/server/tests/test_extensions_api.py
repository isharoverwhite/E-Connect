...
def test_execute_external_device_command_secondary_retry_no_longer_raises_name_error(monkeypatch):
    ...
    monkeypatch.setattr(runtime_module, "time", Mock(sleep=Mock()))

    # After MAX_COMMAND_ATTEMPTS retries the reconcile probe runs once.
    # The probe returns state that doesn't match (brightness=40 vs 115),
    # and since discovery succeeds the error is raised with mark_offline=False.
    from app.services.external_runtime import ExternalDeviceRuntimeError
    ...
    assert exc_info.value.mark_offline is False
