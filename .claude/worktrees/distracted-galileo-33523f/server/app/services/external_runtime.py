...
YEELIGHT_MAX_COMMAND_ATTEMPTS = 5
YEELIGHT_ONLINE_CHECK_TIMEOUT_SECONDS = 0.5
YEELIGHT_RETRY_INTERVAL_SECONDS = 0.05
...
def _is_yeelight_reachable(host: str) -> bool:
    try:
        conn = socket.create_connection((host, YEELIGHT_PORT), timeout=YEELIGHT_ONLINE_CHECK_TIMEOUT_SECONDS)
        conn.close()
        return True
    except OSError:
        return False
...
def _execute_yeelight_command(device: ExternalDevice, command: dict[str, Any]) -> ExternalRuntimeExecutionResult:
    ...
    if diagnostics is not None:
        _refresh_yeelight_discovery(host, diagnostics)

    last_error: ExternalDeviceRuntimeError | None = None

    for attempt in range(YEELIGHT_MAX_COMMAND_ATTEMPTS):
        try:
            ...
            last_error = exc
            if attempt < YEELIGHT_MAX_COMMAND_ATTEMPTS - 1:
                if diagnostics is not None:
                    diagnostics.add("retry_attempt", attempt=attempt + 1, error=str(exc))
                if exc.connection_failed and not _is_yeelight_reachable(host):
                    _emit_yeelight_diagnostics(diagnostics, host=host, stage="command_failed_offline_early")
                    raise exc  # Device is offline — abort retries immediately.
                time.sleep(YEELIGHT_RETRY_INTERVAL_SECONDS)

    assert last_error is not None

    # All attempts failed. Probe state: command may have reached the lamp.
    ...
    if not last_error.connection_failed:
        ...
        if discovery is not None:
            ...
            raise ExternalDeviceRuntimeError(
                f"Yeelight command failed after {YEELIGHT_MAX_COMMAND_ATTEMPTS} attempts (device is online).",
                mark_offline=False,
            ) from last_error
...
def _reconcile_yeelight_command_after_failure(
    *,
    ...
    # Probe immediately — retries already provided settling time.
    try:
...
def _execute_yeelight_command_direct(
    *,
    ...
    with _YeelightLanSession(host, diagnostics=diagnostics) as session:
        ...
        # Query state immediately — Yeelight firmware responds to get_prop
        # with the new target state right after accepting the command.
        try:
