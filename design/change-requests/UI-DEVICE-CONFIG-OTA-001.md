...
This slice also upgrades the saved-config contract so managed-device OTA reconfiguration is tracked as a time-ordered config history keyed by config/build id, keeps only the newest three entries, allows the admin to name each saved config, and shows which board that config is assigned to.
