...
- `Register via HTTP`, `HTTP Force Re-pair`, `Register Normal`, và `Force Re-pair` đều tạo hoặc cập nhật pending device thật trên backend.
- Nếu bạn dừng test mà không `Approve Device`, `Reject Device`, hoặc `Unpair Device`, WebUI thật vẫn sẽ tiếp tục thấy request pair đó.
- Trước khi kết thúc một vòng test, nên dọn trạng thái bằng `Reject Device` hoặc `Unpair Device`, rồi `Generate Fresh Identity` cho vòng sau.
...
### E. Cleanup sau khi test

Để tránh để lại `pair giả` trên web UI:

1. Nếu board đang ở discovery mà không muốn giữ lại, bấm `Reject Device`.
2. Nếu board đã được approve và chỉ dùng để test tạm thời, bấm `Unpair Device`.
3. Bấm `List Pending` để xác nhận device đó không còn nằm trong pending list.
